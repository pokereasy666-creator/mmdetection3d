...
后续收尾审计见 `BLOCK1_CLOSEOUT_AND_4GPU_20260903.md`。本文件保留原次运行
证据；sanity 阶段通过不等于块1整体关闭。下文“仅本地”描述的是总结形成时状态，
三项提交内容现已发布，原本地 SHA 与发布 SHA 的映射见收尾审计。
...
| 报告形成时的发布状态 | 上述脚本提交当时仅在本地；后续发布记录见收尾审计 |
...
本次单卡受控 sanity 可以归档，块1整体尚不能关闭。后续四卡验收须单独留证；
按 2026-09-03 收尾决议可与首个 EP 正式 run 合并，以 G1–G4 闸门推进，至少检查：
