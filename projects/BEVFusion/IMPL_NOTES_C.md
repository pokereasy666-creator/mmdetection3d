...
inputs = [img_bev (B,80,H,W), lidar_bev (B,256,H,W)]        # baseline ConvFuser order
 ...
 ├ cross-attention (heads=8, head_dim=32) — SCALED-COSINE (A15): q,k L2-normed per head,
 │     logits = g·(q̂·k̂), g=exp(λ) learnable per head (init √32, clamp 4√32). via SDPA, FULL 180.
...
| **A6** | Attention scaling — **SUPERSEDED by A15 (scaled-cosine).** The plain `1/√head_dim` dot-product scaling let logits grow with feature magnitude (`√d·mag²`), which is the BUG-2 root cause. | See A15: q,k are L2-normalised per head and `logits = g·(q̂·k̂)`, decoupling the scale from feature magnitude. (Implementation folds `g·√head_dim` into q so SDPA's default `1/√head_dim` yields net `g·cos` — torch-2.0.x has no `scale` kwarg.) |
...
| **A15** | **Scaled-cosine attention (DEVIATION).** q,k L2-normalised per head; `logits = g·(q̂·k̂)`, `g=exp(λ)` learnable **per head**, init `g=√head_dim≈5.66`, clamp `λ≤ln(4√d)` ⇒ `g_max≈22.6`. Folded as `q̂·g·√d` so SDPA's default `1/√d` gives net `g·cos` (torch-2.0.x has no `scale` kwarg). New params: `fusion_layer.logit_scale` (8,). | **BUG-2 fix.** Eq.3 is bare `softmax(QK/√C)`; under our C=256 / BN+ReLU feature magnitudes the dot-product logits scaled with magnitude (`√d·mag²`) → a peaked softmax broadcast a few high-value image cells across all 32400 BEV cells, blowing `|vhat|` up **7–139×** (Stage-A: |attn_out|=76131 vs |i_gb|=3283 at entropy 7.7; ~2.5× at entropy 10.3). L2-norm decouples logits from magnitude ⇒ no peaking. **Modality-neutral** (same op on q=LiDAR and k=image; `g` is a per-head sharpness knob, not a per-modality gate; V=`i_gb` untouched ⇒ camera never suppressed; ≠ γ). Ceiling `4√d` keeps selectivity but stays ~4.4× below the CLIP/Swin-V2 ceiling of 100 (one-hot over 32400). **Does NOT fix the i_gb≈10× vgb input floor** (V untouched) → residual `raw_ratio≈2.5`; if LiDAR is drowned, add a separate input GroupNorm on both i_gb,v_gb (not pre-applied). `‖W_O‖` left alone (Kaiming-init Frob ≈9.2 for 256→256; operator norm sub-unit; data: ×0.57 attenuation). Gate: `raw_ratio` O(1) & stable, `attn_entropy` not collapsing, per-head `g` not pinned at `g_max`, `rel_cam>0.05`. |
